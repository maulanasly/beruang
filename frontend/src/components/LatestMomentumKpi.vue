...
const cards = computed(() => {
  ...
  if (props.asset === 'term-deposits') {
    return [
      ...
      { label: t('kpi.latestProratedInterest'), value: formatter.formatCellValue('prorated_interest', row.prorated_interest) },
      { label: t('kpi.currentApy'), value: formatter.formatCellValue('apy', summary.apy) },
    ]
  }
  ...
  if (props.asset === 'stocks') {
    return [
      { label: dateLabel, value: formatter.formatCellValue('date', row.date) },
      { label: t('kpi.latestMom'), value: formatter.formatCellValue('mom_return', row.mom_return) },
      { label: t('kpi.latestRoi'), value: formatter.formatCellValue('roi', summary.roi) },
      { label: t('kpi.latestXirr'), value: formatter.formatCellValue('xirr', summary.xirr) },
    ]
  }
