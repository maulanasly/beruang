    ...
    return html`<div class="card">
        ...
        <div style="display:flex; gap:16px; align-items:center; flex-wrap:wrap; margin-top:8px">
            <svg viewBox="0 0 180 180" width="180" height="180" role="img" aria-label=${t(locale, 'overview.proportion')}>
                <circle cx="90" cy="90" r=${r} fill="none" style="stroke:var(--chart-track)" stroke-width="28" />
                ${segs.map(s => html`<circle cx="90" cy="90" r=${r} fill="none"
                    style=${`stroke:${s.color}`} stroke-width="28"
                    stroke-dasharray=${`${(s.frac * c).toFixed(2)} ${c.toFixed(2)}`}
                    stroke-dashoffset=${(-s.offset * c).toFixed(2)}
                    transform="rotate(-90 90 90)" />`)}
                <text x="90" y="86" font-size="15" text-anchor="middle" font-weight="bold" style="fill:var(--ink)">${formatCurrency(total, locale, settings.currency)}</text>
                <text x="90" y="104" font-size="10" text-anchor="middle" style="fill:var(--chart-tick)">${t(locale, 'overview.totalValue')}</text>
            </svg>
